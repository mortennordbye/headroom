...
const rules: LabelRule[] = [
  { id: '1', match: 'Til:98765432101', label: 'Boliglån' },
  { id: '2', match: 'SPOTIFY', label: 'Musikk' },
];

describe('labelRules', () => {
  it('returns the label of a matching rule (case-insensitive substring)', () => {
    expect(ruleLabelFor({ description: 'Til:98765432101' }, rules)).toBe('Boliglån');
  ...
  it('txDisplayName prefers the custom label, else the description', () => {
    expect(txDisplayName({ description: 'Til:98765432101' }, rules)).toBe('Boliglån');
