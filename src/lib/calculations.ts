...
//  • equity ≥ 10% of the purchase price (i.e. LTV ≤ 90%) — the floor since Jan 2025.
...
export const MIN_EQUITY_SHARE = 0.10; // 10% equity → 90% LTV
...
export interface BorrowingCapacity {
  ...
  maxPriceFromLtv: number;    // equity / 0.10 (the 10%-equity / 90%-LTV cap)
...
export function calcBorrowingCapacity(
  ...
  const maxPriceFromLtv = eq / MIN_EQUITY_SHARE; // eq must be ≥ 10% of price
