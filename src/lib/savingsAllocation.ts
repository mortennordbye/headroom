...
export interface SavingsAllocation {
  id: string;
  /** Share of the savings target, in percent. Only read when `mode` is 'percent'. */
  percent: number;
  /**
   ...
   * month is fat or lean. 'rest' takes whatever the other rows leave, so the
   * split always adds up without arithmetic on the user's part. Absent reads as
   * 'percent' for stored data.
   */
...
/**
 * Why a plan does not fit its target. The two causes need different words: with
 * a fixed row overdrawing the target the percentages can be perfectly valid, so
 * telling the user to "adjust the percentages" sends them at the wrong control.
 */
export type OverAllocationReason = 'percent' | 'fixed';
...
export interface AllocationPlan {
  ...
  remainder: number;
  /** What the percentages divide: the target after the fixed-kroner rows. */
  shareTarget: number;
  /** Non-null when the plan does not fit; the UI blocks creating expenses then. */
  overReason: OverAllocationReason | null;
  /** `overReason !== null`, for the many call sites that only need the boolean. */
...
const isRest = (a: SavingsAllocation) => a.mode === 'rest';

/**
 * Resolve an allocation to whole kroner against `target`.
 *
 * Three kinds of row, settled in that order: fixed kroner off the top, then the
 * percentages over what is left, then the 'rest' rows over what the percentages
 * did not claim. Uses largest-remainder within each stage so the rows sum
 * EXACTLY to the allocated share — rounding each row independently would leave
 * the total a krone or two off, which then shows up as a phantom "1 kr
 * unallocated" in the UI.
 *
 * A non-positive target (no residual this month) resolves every share row to 0
 * while keeping the percentages, so the plan survives a lean month unchanged.
 */
export function resolveAllocation(allocations: SavingsAllocation[], target: number): AllocationPlan {
  ...
  const shareTarget = Math.max(0, target - fixedTotal);

  const totalPercent = allocations.reduce((s, a) => (isFixed(a) || isRest(a) ? s : s + pct(a.percent)), 0);
  // Fixed rows alone can exceed the target; that is over-allocation too, but a
  // different one — see OverAllocationReason.
  const overReason: OverAllocationReason | null =
    totalPercent > 100 ? 'percent'
      : fixedTotal > Math.max(0, target) ? 'fixed'
        : null;

  const amounts = allocations.map(a => (isFixed(a) ? Math.round(kr(a.amount)) : 0));

  if (shareTarget > 0) {
    // Exact shares, then floor, then hand the lost kroner to the largest
    // fractions. Only the percentage rows take part: a fixed row is already a
    // stated whole number and must not drift by a krone to make someone else's
    // rounding work.
    const exact = allocations.map(a => (isFixed(a) || isRest(a) ? 0 : (shareTarget * pct(a.percent)) / 100));
    allocations.forEach((a, i) => {
      if (!isFixed(a) && !isRest(a)) amounts[i] = Math.floor(exact[i]);
    });
    const percentAllocated = Math.round((shareTarget * Math.min(totalPercent, 100)) / 100);
    let leftover = percentAllocated
      - allocations.reduce((s, a, i) => (isFixed(a) || isRest(a) ? s : s + amounts[i]), 0);
    const order = allocations
      .map((a, i) => ({ i, frac: isFixed(a) || isRest(a) ? -1 : exact[i] - Math.floor(exact[i]) }))
      .filter(o => o.frac >= 0)
    ...
    // Whatever the percentages left behind goes to the 'rest' rows, split evenly
    // between them. More than one rest row is unusual but well-defined, so it
    // needs no guard: two rows asking for "the rest" get half each.
    const restIdx = allocations.map((_, i) => i).filter(i => isRest(allocations[i]));
    if (restIdx.length > 0) {
      const restPool = Math.max(0, shareTarget - percentAllocated);
      const each = Math.floor(restPool / restIdx.length);
      restIdx.forEach(i => { amounts[i] = each; });
      // Even shares, so the largest-remainder tie-break is just "the first rows".
      for (let n = restPool - each * restIdx.length, k = 0; n > 0; n--, k++) amounts[restIdx[k]] += 1;
    }
  }
