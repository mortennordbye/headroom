...
- **BudgetPage historical envelope math (HISTORY_PLAN §5.3).** The header picker now unifies the balance-page time machine (§5.2 shipped), but Budget still uses its own `currentMonth` model. Past Budget months use *today's* fixed-expense amounts for envelopes rather than the recorded `snapshot.fixedExpenses`. **Why deferred:** Budget's envelope math is `currentMonth`-driven, a separate wiring from the balance-page `historyMonth`. **What unblocks:** read `balanceSnapshots[currentMonthKey].fixedExpenses` in the envelope calc when viewing a past month. **Where:** `src/pages/BudgetPage.tsx`, `src/lib/envelopes.ts`.
