...
- **Deepen the time machine.** Mostly addressed by `HISTORY_PLAN.md` phases 1-3: (a) stepper state lifted to shared `historyMonth` ✅; (b) AssetPage projections now use the snapshot's `assumptions`/mortgage rate/term (v1 snapshots still fall back to live — a muted "not recorded" treatment for v1 is the remaining polish); `recommendedInvestment` is still the live month-coupled figure (separate item below); (c) PensionPage contributions now resolve at the viewed month via `calcActiveGrossAnnual` ✅; (d) hand-entered historical backfill shipped via `HistoryManagerModal` ✅.
- **Derived history surfaces still to build (HISTORY_PLAN §6.4, §6.5).** Phases 6.1-6.3 shipped (egenkapital table, actual LTV, mortgage plan-vs-actual). Remaining: (6.4) non-mortgage debt payoff-vs-plan — per-debt actual balance from `snapshot.debts` vs a `planPayoff` curve, surfaced in `DebtSection`; and (6.5) per-account savings history + OTP/IPS balance trend charts on AssetPage/PensionPage. **Why deferred:** same snapshot-derived pattern as the shipped surfaces, just more charts; stopped to keep the shipped ones verified. **What unblocks:** a `paydownVsPlan`-style lib fn against `planPayoff` for debts, then chart components mirroring `PaydownVsPlanChart`/`EquityHistoryTable`. **Where:** `src/lib/debt.ts`, `src/components/DebtSection.tsx`, `src/pages/AssetPage.tsx`, `src/pages/PensionPage.tsx`.
- **Merge the header month picker with the balance-page stepper (HISTORY_PLAN §5.2).** Two month controls coexist: the Dashboard/Budget `currentMonth` picker (`MONTH_SCOPED_ROUTES` in `src/components/Layout.tsx`) and the balance-page `BalanceHistoryBar` (driving `historyMonth`). **Why deferred:** each is coherent on its own pages; unifying is UX work needing browser iteration, not correctness. **What unblocks:** decide the single header model (picker drives `currentMonth` on Dashboard/Budget, `historyMonth` on balance pages, stepping only recorded months) and fold `BalanceHistoryBar` into the header. Also lets BudgetPage past months use `snapshot.fixedExpenses` for envelope math. **Where:** `src/components/Layout.tsx`, `src/components/BalanceHistoryBar.tsx`, `src/pages/BudgetPage.tsx`.
...
- **Fold the scalar net-worth editor into the History manager.** Phase 2 of `HISTORY_PLAN.md` added `HistoryManagerModal` (Settings → Data management) which backfills full manual snapshots for any month, superseding the rolling-12-month scalar `NetWorthHistoryModal`. The scalar editor still exists on the Dashboard unchanged. **Why deferred:** the two are opened from different pages with different affordances; merging is UX work, not needed for the manager to function. **What unblocks:** decide whether the Dashboard "Edit history" opens the manager instead, then retire `NetWorthHistoryModal`. **Where:** `src/components/NetWorthHistoryModal.tsx`, `src/components/HistoryManagerModal.tsx`, `src/pages/DashboardPage.tsx`.
...
- **Net-worth history editor covers a rolling 12-month window** matching the Dashboard chart. Superseded for backfill by the History manager (Settings → Data management), which edits any month via full snapshots; the scalar Dashboard editor still has the 12-month window (see the "Fold the scalar net-worth editor" item above).
